...
**📌 2026-05-01 社長判断：rotate不要（実害発生条件が成立しないため）**

社長との議論で以下を確認・合意：
- 攻撃成立には3点セット必要：`refresh_token` + `EBAY_APP_ID`(Client ID) + `EBAY_CERT_ID`(Client Secret)
- 流出したのは1点（refresh_token）のみ。残り2点は `.env` で守られていて GitHub には流出していない
- → **3点が揃う条件が成立しない＝実害発生条件なし**
- 10〜15分のrotate作業の実利が低いため、社長判断で取り下げ（git履歴削除も同様の論理で取り下げ）

**この判断が成立する3つの前提条件（崩れたら即rotate必須）：**
1. `.env` が GitHub に流出しない（`.gitignore` の `.env` 系除外を維持）
2. `.env` が他経路（メール・チャット貼付け・PCマルウェア等）で流出しない
3. 機密混入時の検知体制が動いている（`github_backup.py` の検出ロジック＋本フックの警告）

**この判断のトリガー（崩れたら見直し）：**
- `.env` ファイルが何らかの理由で漏洩した／しそうになった
- `.gitignore` から `.env` 系除外パターンが消えた
- `github_backup.py` の検出ロジックが無効化された

---

## 🛡 2026-05-01 追加：PreToolUse hook で機密ファイル作成時に警告

ファイル作成時点で機密ファイル名を検知して警告するフック `.claude/hooks/sensitive_file_guard.py` を追加（settings.local.json 登録済み）。

**動作：**
- Write/Edit ツール起動前に対象パスをチェック
- 機密パターン（token/credentials/secret/api_key/.pem 等）にマッチ
- 拡張子フィルタ（`.py/.md/.html/.txt/.gs/.bat/.ps1` 等は除外＝コード/ドキュメントは誤検出しない）
- `.gitignore` で除外されているか確認
- 除外されていなければ stderr に警告 + 推奨gitignoreパターン提示
- **ブロックはしない**（Claudeが判断して必要なら .gitignore 先行更新）

**3層の防衛線が完成：**
1. PreToolUse hook（ファイル作成時の警告）← 今回追加
2. github_backup.py の機密検出（push直前のブロック・既存）
3. `.gitignore` の包括パターン（**/cache/*token*.json 等・既存）

**ガイドファイル `commerce/ebay/analytics/EBAY_TOKEN_ROTATE_GUIDE.md` の扱い：**
- rotate判断が必要になった時のために残す（削除しない）
- 万が一前提条件1か2が崩れた瞬間、即実行できる状態を維持
