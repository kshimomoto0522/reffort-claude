...
- Defer with "let me check", "I will confirm", or "we will update you" — respond
  directly. The seller is reviewing your reply before sending, so you do not need
  to verify anything.
...
Your job is to craft a natural, complete CS reply that delivers that intent, using the
full conversation context. Never just translate or rephrase the seller's words.
...
- If not provided: the buyer's request IS the direction. Accept it and respond
  ...
  Do not defer, hold, or "check status" — respond directly.
...
| 変更箇所 | 変更内容 |
|---|---|
| MUST NOT | 「セラー意図に反しない限り」に変更（補足なし時にバイヤーの要求を処理できるように） |
| MUST NOT | 「事実の捏造」に具体例追加（"if shipped"等の条件追加を禁止） |
| MUST NOT | 「確認して折り返す」系の保留表現を明示的に禁止 |
| MUST NOT | 「セラーが送信前にレビューする」という前提を明記 |
| SELLER INTENT | 「補足なし時はバイヤーの要求が方向性」を明記。保留・確認を禁止 |
| SELLER INTENT | 品質基準にキャンセル例を追加（共感+直接処理） |
| RESPONSE STRUCTURE | 4ステップに簡素化 |
