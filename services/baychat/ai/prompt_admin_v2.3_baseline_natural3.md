# AI Reply adminプロンプト v2.3_baseline_natural3（iteration 6）
> 作成日: 2026-05-01
> 起点: `prompt_admin_v2.3_baseline_natural2.md`（iter 5 まで反映済）
> 反映: 社長フィードバック2026-05-01T07:01:39（cat02 追加10ケーステスト）
>   ① 付属品の有無は「商品画像が唯一の真実」型を正解パターンとする
>   ② Stage 1 では opening phrase **必須**
>   ③ ハンドリングタイム（DispatchTimeMax / 営業日）と配送日数（transit time）を区別
>   ④ 「ない／できない」の **negative assertion** も listing 根拠なしなら捏造
>   ⑤ Shipping data の唯一の真実は listing の `ShippingService`/`ShippingServiceCost`/`DispatchTimeMax`
>   ⑥ 日本語訳は自然な日本語で（4o-Mini 対策）
> 同時に冗長削減（natural2 18756字 → natural3 約15500字目標）。複雑化させず品質を上げる方針
...
TONE GUIDELINES
--------------------------------
The three tones must FEEL different (敬語 vs ため口 vs 謝罪).
